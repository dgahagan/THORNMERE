...
export function initDb(raw) {
  ...
  if (raw.starters) DB.starters = raw.starters;
  return DB;
}

// loader(path) must return parsed JSON for a path like 'data/items.json'
export async function loadAll(loader) {
  const [races, classes, items, spells, songs, monsters, balance, starters] = await Promise.all(
    ['races', 'classes', 'items', 'spells', 'songs', 'monsters', 'balance', 'starters']
      .map(n => loader(`data/${n}.json`)));
  const maps = {};
  await Promise.all(MAP_IDS.map(async id => { maps[id] = await loader(`data/maps/${id}.json`); }));
  return initDb({ races, classes, items, spells, songs, monsters, maps, balance, starters });
}
