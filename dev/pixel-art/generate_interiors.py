...
Usage (from within distrobox, dev/pixel-art/):
  ...
  python3 generate_interiors.py                   # all 9 interiors (6 seeds each = 54 imgs)
  python3 generate_interiors.py --id int_temple   # one interior

GPU is only touched when generating (bridged to the host via distrobox-host-exec);
crushing and manifest writes run in this container. Winners are picked by a human
after a worker session builds comparison sheets — no self-judging.
"""
import argparse
import json
import subprocess
...
def main() -> None:
    ...
    if args.id:
        ...
        entries = [e for e in entries if e.get("art_class") == "interior"]

    if not entries:
        print("No interior entries to process.")
        return
