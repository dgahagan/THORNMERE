#!/usr/bin/env python3
"""generate_feral.py — feral v2 re-prompt variant of generate.py.

Identical to generate.py except CANDIDATES_DIR points to candidates-feral/
so v2 raws and crushed PNGs never clobber the v1 candidates/ set.

Usage (from within distrobox, dev/pixel-art/):
  python3 generate_feral.py --id mon_rat
  python3 generate_feral.py --id mon_hound
  python3 generate_feral.py --id mon_moth
  python3 generate_feral.py --id mon_blob
"""
import argparse
import json
import subprocess
...
CANDIDATES_DIR = PIXELART_DIR / "candidates-feral"
REQUEST_PATH = PIXELART_DIR / "gen_request_feral.json"
