#!/usr/bin/env python3
"""generate.py — manifest-driven candidate generator for the Thornmere AI art pipeline.

Reads data/art/gen-manifest.json, generates raw PNGs via the GPU host (through
the distrobox-host-exec bridge), crushes them to the game sub-palette locally,
and updates manifest status incrementally after every sprite.

Usage:
  python3 generate.py                      # all entries with status "pending"
  python3 generate.py --id mon_sorcerer    # one entry (any status)
  python3 generate.py --force              # re-generate even if status != pending
  python3 generate.py --dry-run            # print plan without running

GPU is only touched when generating; crushing and manifest writes run in this
...
CANDIDATES_DIR = PIXELART_DIR / "candidates"
REQUEST_PATH = PIXELART_DIR / "gen_request.json"
...
def generate_and_crush(entries: list[dict], manifest: dict, dry_run: bool) -> None:
    ...
    # --- Phase 1: build host_gen request ---
    request_entries = []
    ...
    # --- Phase 3: crush + update manifest incrementally ---
    print("Crushing raws …")
    for entry in entries:
...
def main() -> None:
    ...
    ap.add_argument("--force",   action="store_true",
                    help="Process entry even if status != pending")
