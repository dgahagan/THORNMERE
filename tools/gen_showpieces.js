...
const bosses = ['candleking', 'choir_eldest', 'mock_king', 'maldrec'];
const fns = [candleking, choirEldest, mockKing, maldrec];
for (let i = 0; i < bosses.length; i++) {
  for (let f = 0; f < 3; f++) {
    sprites[`mon_${bosses[i]}_${String.fromCharCode(97 + f)}`] = fns[i](f);
  }
}
...
for (const name of bosses) {
  anims[`mon_${name}`] = { frames: [
    ...
    { sprite: `mon_${name}_c`, ms: 400 },
  ]};
}

const variants = {
  tallow_king:   { base: 'mon_candleking' },
  choir_eldest:  { base: 'mon_choir_eldest' },
  mock_king:     { base: 'mon_mock_king' },
  maldrec:       { base: 'mon_maldrec' },
  maldrec_image: { base: 'mon_maldrec' },
};

writeFileSync(
  join(ROOT, 'data', 'art', 'monsters3.json'),
  JSON.stringify({ comment: 'Boss showpieces 96×80 × 3 frames. Generated by tools/gen_showpieces.js', sprites, anims, variants }, null, 2)
);
